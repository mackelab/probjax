...
def find_ancestors_jax(mask, node):
    ...
    def body_fn(carry, i):
        ...
        def inner_body_fn(carry, j):
            ...
            is_ancestor, stack = jax.lax.cond(
                cond, true_fn, false_fn, is_ancestor, stack
            )
            return (is_ancestor, stack), None

        (is_ancestor, stack), _ = jax.lax.scan(
            inner_body_fn, (is_ancestor, stack), jnp.arange(num_nodes)
        )
        return (is_ancestor, stack), None

    (is_ancestor, stack), _ = jax.lax.scan(
        body_fn, (is_ancestor, stack), jnp.arange(num_nodes)
    )
...
def faithfull_mask(base_mask, condition_mask, conditioned_nodes="unchanged"):
    ...
    def body_fn(carry, i):
        ...
        def condition_case(base_mask, condition_mask):
            ...
            parents_of_children_of_i = base_mask & (
                children_of_i[:, None] & ~children_of_i[None, :]
            )
        ...
        base_mask, condition_mask = jax.lax.cond(
            condition_mask[i],
            condition_case,
            uncondition_case,
            base_mask,
    ...
    (base_mask, condition_mask), _ = jax.lax.scan(
        body_fn, (base_mask, condition_mask), jnp.arange(num_nodes)
    )
...
def min_fill_heuristic(G, I, S, M, top_mode=0):  # noqa: E741
    ...
    num_parents = marked_parents * (num_edges_added == min_val) + (I.shape[0] + 1) * (
        num_edges_added != min_val
    )
...
def convert_to_networkx(mask):
    ...
    return nx.from_numpy_array(
        mask - jnp.eye(mask.shape[0]), create_using=nx.DiGraph
    ).reverse()
