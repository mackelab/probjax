    ...
    *residual_refs,
    ...
    def body(start_k, carry):
        ...
        if s_ref is not None or mask_fn is not None:
            mask = (
                None if mask_fn is None else mask_fn(span_q[:, None], span_k[None, :])
            )
    ...
    if index_offset_size_ref is not None:
        ...
        o, m_i, l_i = lax.fori_loop(
            0, pl.cdiv(kv_seq_len, block_k), body, (o, m_i, l_i)
        )
...
def flash_attention(
    ...
    Returns:
        The attention output jax.Array of shape [batch_size, target_length, num_heads, per_head_dim].
    """
...
def _flash_attention_impl(
    ...
    Returns:
        If output_activations is False:
            jax.Array of shape [batch_size, target_length, num_heads, per_head_dim]
        If output_activations is True:
            (jax.Array of shape [batch_size, target_length, num_heads, per_head_dim], (residuals, ...))
    """
    ...
    if num_stages is None:
        num_stages = (
            2
            if bias is None and jnp.float32 not in (query.dtype, key.dtype, value.dtype)
            else 1
        )
    ...
    out_specs = pl.BlockSpec(
        (None, block_q, None, block_d), lambda i, j, k: (j, i, k, 0)
    )
    ...
    pallas_out = pl.pallas_call(
        ...
        in_specs=in_specs,
        out_specs=out_specs,
        compiler_params=TritonCompilerParams(
...
def _mha_backward_kernel_dkdv(
    ...
    *,
    ...
    if index_offset_size_ref is not None:
        ...
        dv, dk = lax.fori_loop(
            0, pl.cdiv(q_seq_len, block_q), inner_loop_dkdv, (dv, dk)
        )
...
def _mha_backward(
    ...
    if jnp.float32 in (
        ...
        k.dtype,
        v.dtype,
        jnp.bfloat16 if bias is None else bias.dtype,
    ):
        block_q = block_k = 32
    ...
    num_kv_blocks = pl.cdiv(kv_seq_len, block_k)
    num_q_blocks = pl.cdiv(q_seq_len, block_q)
    q_index_offset = q_index_offset_spec = q_index_offset_size = (
        q_index_offset_size_spec
    ) = None
    kv_index_offset = kv_index_offset_spec = kv_index_offset_size = (
    ...
    if num_stages is None:
        num_stages = (
            2 if bias is None and jnp.float32 not in (q.dtype, k.dtype, v.dtype) else 1
        )
