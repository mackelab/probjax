...
mlir.register_lowering(
    _flash_mha_fwd_p,
    mlir.lower_fun(_flash_mha_fwd_hlo),
    platform="gpu",
)

mlir.register_lowering(
    _flash_mha_bwd_p,
    mlir.lower_fun(_flash_mha_bwd_hlo),
    platform="gpu",
)
...
def mha_fwd_batch(vector_arg_values, batch_axes, **kwargs):
    ...
    if mapped == (True, True, True):
        ...
        raise NotImplementedError(
            "MHA fwd only support vmapping over q or (q,k,v) for now, got batch axes "
            + str(batch_axes)
        )
...
def mha_bwd_batch(vector_arg_values, batch_axes, **kwargs):
    ...
    if mapped == (True, True, True, True, True, True):
        ...
        do, q, k, v, o, lse = [
            squish(x, axis) for x, axis in zip(vector_arg_values, batch_axes)
        ]
        ...
        raise NotImplementedError(
            "MHA bwd only support vmapping over q or (q,k,v) for now, got batch axes "
            + str(batch_axes)
        )
...
def flash_mha(q, k, v, softmax_scale=None, is_causal=False, window_size=(-1, -1)):
    ...
    o = _flash_mha_vjp(
        ...
        k,
        v,
        dict(softmax_scale=softmax_scale, is_causal=is_causal, window_size=window_size),
    )
    return o
