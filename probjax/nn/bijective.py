...
def _rational_quadratic_spline_fwd(
    ...
    if x_min is not None and y_min is not None:
        ...
        y_below = jnp.where(
            jnp.isnan(slope_below_bounded), 
            y_below_unbounded, 
            y_below_bounded
        )
        logdet_below = jnp.where(
            jnp.isnan(slope_below_bounded),
            logdet_below_unbounded,
            logdet_below_bounded
        )
    else:
    ...
    if x_max is not None and y_max is not None:
        ...
        y_above = jnp.where(
            jnp.isnan(slope_above_bounded), 
            y_above_unbounded, 
            y_above_bounded
        )
        logdet_above = jnp.where(
            jnp.isnan(slope_above_bounded),
            logdet_above_unbounded,
            logdet_above_bounded
        )
    else:
        # Fall back to the unbounded version
...
def _rational_quadratic_spline_inv(
    ...
    logdet_below_unbounded = -jnp.log(knot_slopes[0])  # = jnp.log(slope_below_unbounded)
...
def rational_quadratic_spline(
    ...
    if not bounded:
        ...
        y, _ = _rational_quadratic_spline_fwd(x, x_pos, y_pos, knot_slopes, range_min_x, range_max_x, range_min_y, range_max_y)
    return y
...
def inv_rational_quadratic_spline(
    ...
    if not bounded:
        ...
        y, log_det = _rational_quadratic_spline_inv(x, x_pos, y_pos, knot_slopes, range_min_x, range_max_x, range_min_y, range_max_y)
...
def _piecewise_linear_spline_fwd(
    ...
    if (x_min is not None) and (y_min is not None):
        ...
        y_below = jnp.where(jnp.isnan(slope_below_bounded),
                            y_below_unbounded,
                            y_below_bounded)
        logdet_below = jnp.where(jnp.isnan(slope_below_bounded),
                                 logdet_below_unbounded,
                                 logdet_below_bounded)
    else:
    ...
    if (x_max is not None) and (y_max is not None):
        ...
        y_above = jnp.where(jnp.isnan(slope_above_bounded),
                            y_above_unbounded,
                            y_above_bounded)
        logdet_above = jnp.where(jnp.isnan(slope_above_bounded),
                                 logdet_above_unbounded,
                                 logdet_above_bounded)
    else:
        # Fallback to unbounded behavior
...
def _piecewise_linear_spline_inv(
    ...
    if (y_min is not None) and (x_min is not None):
        ...
        x_below = jnp.where(jnp.isnan(slope_below_bounded),
                            x_below_unbounded,
                            x_below_bounded)
        logdet_below = jnp.where(jnp.isnan(slope_below_bounded),
                                 logdet_below_unbounded,
    ...
    if (y_max is not None) and (x_max is not None):
        ...
        x_above = jnp.where(jnp.isnan(slope_above_bounded),
                            x_above_unbounded,
                            x_above_bounded)
        logdet_above = jnp.where(jnp.isnan(slope_above_bounded),
                                 logdet_above_unbounded,
...
def piecwise_linear_spline(
    ...
    if not bounded:
        ...
        y, _ = _piecewise_linear_spline_fwd(x, x_pos, y_pos, range_min_x, range_max_x, range_min_y, range_max_y)
    return y

def _pieceswise_linear_spline_inv(params: Array, x: Array, range_min_x=-1.0, range_max_x=1.0, range_min_y=-1.0, range_max_y=1.0, min_bin_size=1e-4, bounded=False):
    x_pos, y_pos = jnp.split(params, 2, axis=-1)
    ...
    if not bounded:
        ...
        y, log_det = _piecewise_linear_spline_inv(x, x_pos, y_pos, range_min_x, range_max_x, range_min_y, range_max_y)
    return y, jnp.squeeze(log_det)
