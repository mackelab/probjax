...
_BIVARIATE_INVERSE_REGISTRY = {
    ...
    jax.lax.div_p: (
        jax.lax.mul_p,
        lambda x, y, **params: jax.lax.div_p(y, x, **params),
    ),  # a / b = c -> a = c * b -> b = a / c
    jax.lax.add_p: (
        jax.lax.sub_p,
        jax.lax.sub_p,
    ...
    jax.lax.pow_p: (
        ...
        lambda x, y, **params: [jax.lax.log_p.bind(x)[0] / jax.lax.log_p.bind(y)[0]],
    ),
    jax.lax.pow_p: (
        lambda x, y, **params: jax.lax.pow_p.bind(
            x, 1.0 / y, **params
...
class InverseProcessingRules:
    def __call__(self, eqn, known_invars, known_outvars):
        ...
        elif eqn.primitive in _CUSTOM_INVERSE_PROCESSING_RULES:
    ...
    def _default_bivariate_inverse(self, eqn, known_invars, known_outvars):
        ...
        inv_primitive = _BIVARIATE_INVERSE_REGISTRY[primitive]
        if isinstance(inv_primitive, Primitive):
            subfuns, bind_params = inv_primitive.get_bind_params(eqn.params)
    ...
    def _default_forward_processing(self, eqn, known_invars, known_outvars):
        ...
        if not eqn.primitive.multiple_results:
            output_vals = [outvals]
        return eqn.outvars, output_vals
