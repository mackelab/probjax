...
def test_watson_logpdf_symmetry_and_uniform_limit():
    ...
    logpdf_uniform_ref = watson.logpdf(_unit_vector(jnp.array([0.1, -0.3, 0.95])), mean_direction, kappa=0.0)
...
def test_bingham_samples_live_on_sphere():
    ...
    samples = bingham.rvs(key, orientation=orientation, concentration=concentration, shape=(64,))
    norms = jnp.linalg.norm(samples, axis=-1)
    assert jnp.allclose(norms, 1.0, atol=1e-6)
...
def test_bingham_natural_parameter_shape():
    ...
    params = bingham.natural_parameters(orientation=orientation, concentration=concentration)
...
def test_bingham_fit_estimates_orientation():
    ...
    samples = bingham.rvs(key, orientation=orientation, concentration=concentration, shape=(512,))
