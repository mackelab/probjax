...
def test_inverse(invertible_function: Callable):
    ...
    assert jnp.allclose(x, inv_y, atol=1e-3, rtol=1e-3), "Inverse function value is not correct."
...
def test_inverse_and_logabsdet(invertible_function: Callable):
    ...
    assert jnp.allclose(logabsdet, logabsdet_true, atol=1e-3, rtol=1e-3), "Logabsdet is not correct."
